...
class DeveloperHubApp:
    ...
    def create_widgets(self):
        ...
        self.reg_menu.add_command(label="Email Key to User", command=self.email_selected_key)
        ...
        if cmd == "git":
            if subcmd == "add":
                ...
                if "-A" in files_to_add:
                    ignore_dirs = {".git", ".wrangler", "build", "dist", "Backups", "__pycache__", ".venvxs", ".venvxr", ".venv"}
    ...
    def email_selected_key(self):
        selected = self.tree.selection()
        if selected:
            item_values = self.tree.item(selected[0], "values")
            if len(item_values) > 5:
                reg_key = item_values[3]
                reg_name = item_values[4]
                reg_email = item_values[5]
                app_name = item_values[1] or "XenoScript"
                
                if not reg_email:
                    from tkinter import messagebox
                    messagebox.showwarning("Missing Email", "This registration does not have an email address associated with it.")
                    return
                
                # Construct email body
                subject = f"{app_name} Commercial Version License Key"
                body = f"Hi {reg_name},\n\nThank you for registering {app_name}!\n\nHere is your registration key to activate the full commercial version:\n\n{reg_key}\n\nTo activate, open {app_name}, click 'Enter Registration Key' on the startup screen (or go to Help -> Registration...), and paste your registration key.\n\nBest regards,\nXenoHead Dev Team"
                
                # URL-encode subject and body for mailto link
                import urllib.parse
                mailto_url = f"mailto:{reg_email}?subject={urllib.parse.quote(subject)}&body={urllib.parse.quote(body)}"
                
                # Open system mail client
                import webbrowser
                webbrowser.open(mailto_url)
                self.log(f"📧 Opened system mail client to email key to {reg_email}")
