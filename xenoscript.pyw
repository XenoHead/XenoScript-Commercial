...
"""
    ...
    def download_and_install_github_update(self):
        """Download the latest update setup file and run the external updater helper."""
        try:
            import urllib.request
            import tempfile
            import subprocess
            ...
            # 2. Download the setup executable to system temp directory
            ...
            req_download = urllib.request.Request(asset_url, headers=download_headers)
            with urllib.request.urlopen(req_download, timeout=45) as resp, open(dest_path, "wb") as f_out:
                f_out.write(resp.read())

            # 3. Launch the external updater
            ...
            if not os.path.exists(updater_exe):
                ...
                if os.path.exists(updater_py):
                    subprocess.Popen([sys.executable, updater_py, "--installer", dest_path, "--pid", str(os.getpid())])
                    sys.exit(0)
                else:
                    return {"error": f"XenoScriptUpdater.exe not found in {current_dir}."}
            else:
                subprocess.Popen([updater_exe, "--installer", dest_path, "--pid", str(os.getpid())])
                sys.exit(0)
