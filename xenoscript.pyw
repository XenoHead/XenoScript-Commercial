...
class BackendAPI:
    def __init__(self):
        ...
        if use_portable:
            ...
            self.notes_file = os.path.join(run_dir, "MyNotes.txt")
        else:
            self.app_data_dir = os.path.join(os.path.expanduser("~"), "Documents", "XenoScript")
            self.backup_dir = os.path.join(self.app_data_dir, "Backups")
            self.settings_file = os.path.join(self.backup_dir, "settings.json")
            self.notes_file = os.path.join(self.app_data_dir, "MyNotes.txt")
        ...
        legacy_settings_file = os.path.join(os.path.expanduser("~"), "Documents", "XenoScript", "Backups", "settings.json")
    ...
    def get_registration_info(self):
        ...
        try:
            ...
            if os.path.exists(license_path):
                ...
                if decrypted_str:
                    license_info = json.loads(decrypted_str)
                    if not current_key:
                        current_key = license_info.get("regkey", "")
                    reg_name = license_info.get("regname", "")
                    reg_email = license_info.get("regemail", "")
                    date_registered = license_info.get("date_registered", "")
        except Exception:
            pass
        ...
        if is_registered:
            # Auto-heal: If registered but local details are missing, try fetching from server
            if not reg_name and current_key:
                try:
                    import urllib.request
                    import ssl
                    worker_url = settings.get("licensingWorkerUrl", "https://xenoscript-licensing.skotxeno.workers.dev")
                    ...
                    with urllib.request.urlopen(req, timeout=5, context=context) as response:
                        ...
                        if resp_json.get("success"):
                            reg_name = resp_json.get("regname", "")
                            reg_email = resp_json.get("regemail", "")
                            date_registered = resp_json.get("dateregistered", "")
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10, context=context) as response:
                ...
                if resp_json.get("success"):
                    ...
                    reg_name = resp_json.get("regname", "")
                    reg_email = resp_json.get("regemail", "")
                    date_reg = resp_json.get("dateregistered", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
initialfile="{safe_name}.xsp",
defaultextension=".xsp",
filetypes=[('XenoScript Project', '*.xsp'), ('Legacy KindredScript Project', '*.ksp')]
)
print(file)
"""
...
file = filedialog.askopenfilename(
    parent=root, 
    title="Open Project",
    filetypes=[('XenoScript Project', '*.xsp'), ('Legacy KindredScript Project', '*.ksp'), ('All Files', '*.*')]
)
print(file)
"""
        filepath = run_dialog_script(script)
...
"""
    ...
    def open_manual(self):
        try:
            ...
            if os.path.exists(manual_path):
                import webview
                webview.create_window('XenoScript Manual', url=pathlib.Path(manual_path).as_uri(), js_api=self, width=1024, height=768, text_select=True)
                return True
    ...
    def get_local_copy_if_cloud(self, current_file, local_dir, project_name):
        ...
        try:
            ...
            safe_name = "".join(c for c in project_name if c not in r'\/:*?"<>|')
            local_path = os.path.join(local_dir, f"{safe_name}.xsp")
    ...
    def load_latest_cloud(self, cloud_dir, project_name):
        ...
        target_path = os.path.join(project_cloud_dir, f"{safe_name}.xsp")
        ...
        if os.path.exists(target_path):
            ...
            return {'not_found': True, 'message': f"No file named '{safe_name}.xsp' was found in the cloud directory.\n\nChecked Path: {project_cloud_dir}"}
    ...
    def save_latest_cloud(self, cloud_dir, project_name, content):
        ...
        try:
            ...
            if os.path.exists(target_path):
                timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                backup_name = f"{safe_name}_{timestamp}.xsp"
    ...
    def get_version_info(self):
        # Always use the hardcoded DEFAULT_VERSION as the authoritative "what am I?" answer.
        # The on-disk version.json may be from a previous install and will be stale.
        version_file = os.path.join(current_dir, "version.json")
        # Seed the file on disk if missing (used by display/changelog UI)
    ...
    def check_for_github_updates(self):
        ...
        if token:
            ...
            url = "https://api.github.com/repos/XenoHead/XenoScript-Commercial/contents/version.json"
        else:
            url = f"https://raw.githubusercontent.com/XenoHead/XenoScript-Commercial/main/version.json?t={int(time.time())}"
    ...
    def download_and_install_github_update(self):
        """Pass the download URL to the external updater helper to hard kill and update."""
        try:
            import urllib.request
            import tempfile
            import subprocess
            ...
            # 1. Fetch release details to get the asset ID
            headers = {
            ...
            release_url = "https://api.github.com/repos/XenoHead/XenoScript-Commercial/releases/latest"

            req = urllib.request.Request(release_url, headers=headers)
            with urllib.request.urlopen(req, timeout=10) as resp:
                release_info = json.loads(resp.read().decode("utf-8"))

            # Find the setup asset (usually ends with _Setup.exe)
            setup_asset = None
            ...
            if not setup_asset:
                return {"error": "Could not find setup installer in the latest release."}

            asset_url = setup_asset.get("url")
            asset_name = setup_asset.get("name", "XenoScript_Setup.exe")

            # 2. Launch the external updater
            current_dir = os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else os.path.dirname(os.path.abspath(__file__))
            updater_exe = os.path.join(current_dir, "XenoScriptUpdater.exe")

            args = ["--url", asset_url, "--name", asset_name, "--pid", str(os.getpid())]
            if token:
                args.extend(["--token", token])

            if not os.path.exists(updater_exe):
                # Fallback to python script mode for development/testing
                updater_py = os.path.join(current_dir, "xenoscript_updater.py")
                if os.path.exists(updater_py):
                    subprocess.Popen([sys.executable, updater_py] + args)
                    sys.exit(0)
                else:
                    return {"error": f"XenoScriptUpdater.exe not found in {current_dir}."}
            else:
                subprocess.Popen([updater_exe] + args)
                sys.exit(0)
    ...
    def install_cloud_update(self, cloud_dir):
        ...
        cloud_exe = os.path.join(cloud_dir, "xenoscript.exe")
        if not os.path.exists(cloud_exe):
            cloud_exe_alt = os.path.join(cloud_dir, "XenoScript.exe")
        ...
        try:
            ...
            bat_path = os.path.join(temp_dir, "xenoscript_updater.bat")
            
            with open(bat_path, "w") as f:
                f.write(f"""@echo off
title XenoScript Updater
echo Waiting for XenoScript to close...
...
echo.
echo XenoScript updated successfully!
echo Restarting...
start "" "{current_exe}"
del "%~f0"
""")
    ...
    def get_initial_file(self):
        if len(sys.argv) > 1 and sys.argv[1] != '--run-dialog':
            filepath = sys.argv[1]
            if os.path.exists(filepath) and filepath.lower().endswith(('.xsp', '.ksp')):
                try:
                    with open(filepath, "r", encoding="utf-8") as f:
                        return {'filepath': filepath, 'data': f.read()}
                except Exception as e:
...
if __name__ == '__main__':
    try:
        if os.name == 'nt':
            ...
            _mutex = kernel32.CreateMutexW(None, False, "XenoScriptMutex")
        ...
        if os.name == 'nt' and os.path.exists(icon_path):
            webview.start(icon=icon_path, debug=False, private_mode=False)
        else:
        ...
        try:
            ...
            docs_dir = os.path.join(os.path.expanduser("~"), "Documents", "XenoScript")
            ...
            with open("xenoscript_crash_log.txt", "a", encoding="utf-8") as f:
                f.write(f"\n--- Crash on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ---\n")
                f.write(traceback.format_exc())
        ...
        try:
            import ctypes
            ctypes.windll.user32.MessageBoxW(0, f"XenoScript encountered a fatal error and failed to start.\n\nA crash log has been saved to your Documents/XenoScript folder.\n\nError: {str(e)}", "XenoScript Error", 0x10)
        except:
            pass
        sys.exit(1)