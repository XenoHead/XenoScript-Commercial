      ...
      try {
        ...
        if (row.Authkey && row.Authkey.trim() !== "") {
          if (row.Authkey === authkey) {
            return new Response(JSON.stringify({ success: true, message: "License is active on this device." }), {
              status: 200,
              headers: { ...corsHeaders, "Content-Type": "application/json" }
            });
          } else {
        ...
        return new Response(JSON.stringify({ success: true, message: "Activation successful! thank you for registering." }), {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" }
        });
